
















inline pxrInternal_v0_20__pxrReserved__::TfToken const & to_cpp_ref(
    pxrInternal_v0_20__pxrReserved___TfToken const * rhs)
{
    return *(reinterpret_cast<pxrInternal_v0_20__pxrReserved__::TfToken const * >(rhs));
}


inline pxrInternal_v0_20__pxrReserved__::TfToken & to_cpp_ref(
    pxrInternal_v0_20__pxrReserved___TfToken * rhs)
{
    return *(reinterpret_cast<pxrInternal_v0_20__pxrReserved__::TfToken * >(rhs));
}


inline pxrInternal_v0_20__pxrReserved__::TfToken const * to_cpp(
    pxrInternal_v0_20__pxrReserved___TfToken const * rhs)
{
    return reinterpret_cast<pxrInternal_v0_20__pxrReserved__::TfToken const * >(rhs);
}


inline pxrInternal_v0_20__pxrReserved__::TfToken * to_cpp(
    pxrInternal_v0_20__pxrReserved___TfToken * rhs)
{
    return reinterpret_cast<pxrInternal_v0_20__pxrReserved__::TfToken * >(rhs);
}


inline pxrInternal_v0_20__pxrReserved___TfToken const * to_c(
    pxrInternal_v0_20__pxrReserved__::TfToken const & rhs)
{
    return reinterpret_cast<pxrInternal_v0_20__pxrReserved___TfToken const * >(&(rhs));
}


inline pxrInternal_v0_20__pxrReserved___TfToken const * to_c(
    pxrInternal_v0_20__pxrReserved__::TfToken const * rhs)
{
    return reinterpret_cast<pxrInternal_v0_20__pxrReserved___TfToken const * >(rhs);
}


inline pxrInternal_v0_20__pxrReserved___TfToken * to_c(
    pxrInternal_v0_20__pxrReserved__::TfToken & rhs)
{
    return reinterpret_cast<pxrInternal_v0_20__pxrReserved___TfToken * >(&(rhs));
}


inline pxrInternal_v0_20__pxrReserved___TfToken * to_c(
    pxrInternal_v0_20__pxrReserved__::TfToken * rhs)
{
    return reinterpret_cast<pxrInternal_v0_20__pxrReserved___TfToken * >(rhs);
}


inline pxrInternal_v0_20__pxrReserved___TfToken to_c_copy(
    pxrInternal_v0_20__pxrReserved__::TfToken const & rhs)
{
    pxrInternal_v0_20__pxrReserved___TfToken result;
    pxrInternal_v0_20__pxrReserved___TfToken_copy(&(result), reinterpret_cast<pxrInternal_v0_20__pxrReserved___TfToken const * >(&(rhs)));
    return result;
}
